...
architecture BerlekampMassey of BerlekampMassey is
  ...
  type   states is (reset_FSM, idle, prepare_discrepancy, calculate_discrepancy, set_l, prepare_polynomials, update_polynomials, update_variables, sink_in_update, set_done);
  ...
  begin
    case current_state is
      ...
      when prepare_polynomials =>
        next_state <= update_polynomials;
        
      when update_polynomials =>
        next_state <= update_variables;
        
      when update_variables =>
        next_state <= sink_in_update;
